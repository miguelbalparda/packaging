...
try {
    ...
    $packageList = $opt->getOption('p');
    if (!empty($packageList)) {
        $logger->info(sprintf("Creating archive for the following packages: %s. ", $packageList));
    }
    ...
    foreach ($reader->readMagentoPackages() as $package) {
        $noOfZips += archivePackage($package, $generationDir, $logger, $packageList);
    }

    //archive root package
    $noOfZips += archivePackage($reader->readFromDir(''), $generationDir, $logger, $packageList, $reader->getExcludePaths());
 ...
 * @throws Exception
 */
function archivePackage($package, $generationDir, $logger, $packageList, $excludes = [])
{
    $noOfZips = 0;
    $version = $package->get('version');
    $packageName = str_replace('/', '_', $package->get('name'));
