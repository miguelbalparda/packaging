...
$overrideRepoOption = 'overriderepo';
...
"Usage: php -f create-root.php --
    --source-dir=<path>
    [--type=<$baseOption|$productOption|$projectOption|$overrideRepoOption>] [--wildcard]
    ...
    --type=$baseOption|$productOption|$projectOption|$overrideRepoOption - render the result as a product base or as a product or as a project.
        ...
        --type=$overrideRepoOption allow caller to override repositories node
    ...
    --repo='packages.repository' - which additional custom repository to retrieve packages from: must be given if EE or B2B edition
        is used. repo.magento.com is already used for all editions
    \n"
);
$opt = getopt('', ['source-dir::', 'type::', 'wildcard', 'target-file::', 'set::', 'edition::', 'repo::', 'package-repo-url::']);

$cePackageRepo = !empty($opt['package-repo-url']) ? $opt['package-repo-url'] : 'http://repo.magento.com/';
...
try {
    ...
    $validTypes = [$baseOption, $productOption, $projectOption, $overrideRepoOption, 'default'];
    ...
    switch ($type) {
        ...
        case $projectOption:
            ...
            break;
        case $overrideRepoOption:
            $targetPackage = createDefault($templatePackage, $overrideRepoDefaults, $source);
            break;
        default:
...
function createBase($package, $defaults, $source, $editionCode = 'ce')
{
    ...
    $extraMap = $reader->getRootMappingPatterns();
    if ($editionCode === 'ee') {
        $filterVendor = ['vendor/.htaccess', 'vendor/.htaccess'];
        foreach ($extraMap as $key=>$item) {
            if ($item === $filterVendor) {
                unset($extraMap[$key]);
            }
        }
    }
    $targetPackage->set('extra->map', $extraMap);
...
{
    $pathOfWhiteLists =  __DIR__ . '/../../../../composer_require';
